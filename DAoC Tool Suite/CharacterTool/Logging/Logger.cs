...
namespace DAoCToolSuite.CharacterTool.Logging
{
    public class Logger
    {
        ...
        public void InitalizeLogger()
        {
            lock (thisLock)
            {
                ...
                {
                    Name = "file",
                    FileName = $"{Path.GetDirectoryName(Application.ExecutablePath)}\\DAoCToolSuite.log", //Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)
